...
int game::get_height(display& display){
    return display.get_window_height();
}

int game::get_width(display& display){
...
///    Spot_light[0] = spot_light1;
    ...
    ///basic_shader.load_shaders(vertex_shader, fragment_shader);
    ///basic_shader.bind();
...
void game::update( transform& transform,camera& camera){
...
////Shader.set_uniform_spot_light();
//
//    basic_shader.set_uniform_mat4f("u_mvp", Transformation.get_projected_transformation());
//    basic_shader.set_uniform_mat4f("translate", Transformation.get_transformation());
//    basic_shader.set_uniform_4f("eyepos",Transformation.Camera.get_position().x,Transformation.Camera.get_position().y,Transformation.Camera.get_position().z,1.0f);
//
////Mesh.draw(Shader);
//
//
...
game_object* game::getroot(){
    return &root;
}

void game::delete_textures(){
